summary: How to Create a Livequiz in Android SDK
id: LivequizAndroid
categories: android
tags: Livequiz
status: Published 
...
```
```kotlin
implementation ("com.nudgenow.nudge_android:livequiz:1.0.0")
```
...
- The plugin for Checklists is called `LivequizUi`
- Initialize an instance of `LivequizUi` and pass them into your `NudgeProvider` instance as a list.

```java Java
LivequizUi livequizUi = new LivequizUi();
NudgeProvider nudgeProvider = new NudgeProvider(
        activityContext,
        nudge,
        Arrays.asList(livequizUi)
);

```
```kotlin
val livequizUi = LivequizUi()
...
Duration: 1
### Congrats
You have sucessfully added the Livequiz SDK to you App.